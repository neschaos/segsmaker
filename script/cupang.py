...
class Tunnel:
    ...
    Args:
        ...
        max_retries (int, optional): Default number of times a tunnel will try to relaunch itself \
            if it dies unexpectedly (i.e. not because `stop()` was called). Defaults to 3. \
            Can be overridden per-tunnel via `add_tunnel(..., retries=...)`.
        retry_delay (int, optional): Base delay in seconds before the first retry. Doubles \
            after each failed attempt (exponential backoff), capped at 60s. Defaults to 5. \
            Can be overridden per-tunnel via `add_tunnel(..., retry_delay=...)`.
    ...
    def with_tunnel_list(
        ...
        Args:
            ...
            max_retries (int, optional): Default retry count for tunnels that die unexpectedly. Defaults to 3.
            retry_delay (int, optional): Base backoff delay in seconds. Defaults to 5.
    ...
    def add_tunnel(
        ...
        *,
        ...
        Args:
            ...
            retries (int, optional): How many times to relaunch this specific tunnel if it dies \
                unexpectedly. Defaults to the Tunnel instance's `max_retries`.
            retry_delay (int, optional): Base backoff delay (seconds) for this specific tunnel. \
                Defaults to the Tunnel instance's `retry_delay`.
        ...
        log = self.logger
        log.debug(f"Adding tunnel {command=} {pattern=} {name=} {note=} {retries=} {retry_delay=} {callback=}")
    ...
    def _run(self, tunnel: TunnelDict) -> None:
        """
        Run the tunnel process and monitor its output. Relaunches the tunnel
        automatically (with exponential backoff) if the process dies on its
        own while the Tunnel has not been asked to stop.

        Args:
            tunnel (TunnelDict): The tunnel configuration to run.
        """
        name = tunnel["name"]
        cmd_template = tunnel["command"]
        max_retries = tunnel.get("retries")
        max_retries = self.max_retries if max_retries is None else max_retries
        base_delay = tunnel.get("retry_delay")
        base_delay = self.retry_delay if base_delay is None else base_delay
        ...
        while not self.stop_event.is_set():
            try:
                ...
                cmd = cmd_template.format(port=self.port)
                if not self.WINDOWS:
                    cmd = shlex.split(cmd)
                ...
                log.error(
                    f"An error occurred while running the command: {cmd_template}", exc_info=True
                )

            # a partir daqui o processo encerrou (de proposito ou nao)
            if self.stop_event.is_set():
                break
            ...
            if attempt > max_retries:
                log.error(
                    f"Tunnel '{name}' caiu e excedeu o limite de {max_retries} "
                    f"tentativas de reconexao. Desistindo."
                )
                self.logger.warning(f"⚠️ Tunnel '{name}' encerrado definitivamente apos {max_retries} tentativas.")
                break

            self.logger.warning(
                f"⚠️ Tunnel '{name}' caiu, tentando reconectar "
                f"({attempt}/{max_retries}) em {delay}s..."
            )
            # espera em pequenos passos pra poder abortar rapido se stop() for chamado
            waited = 0
            while waited < delay and not self.stop_event.is_set():
                time.sleep(1)
                waited += 1
            delay = min(delay * 2, 60)  # backoff exponencial, teto de 60s
